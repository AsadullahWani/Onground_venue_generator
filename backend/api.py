...
def search_venues_post(body: VenueSearchRequest):
    ...
    Supports AI summaries via Groq when `include_ai_summary=true`.
    ...
    if body.ai_categorize:
        ...
        if not akey:
            raise HTTPException(
                status_code=422,
                detail="groq_categorize_api_key is required when ai_categorize=true.",
            )
        venues = categorize_venues(venues, groq_api_key=akey)

        # Apply sport filter if requested
        if body.ai_filter_sports:
            before = len(venues)
            venues = filter_by_ai_sport(
                venues,
                sports=body.ai_filter_sports,
                include_unknown=body.ai_include_unknown,
                min_confidence=body.ai_min_confidence,
            )
    ...
    if body.include_ai_summary:
        ...
        if not groq_key:
            raise HTTPException(
                status_code=422,
                detail="groq_api_key is required when include_ai_summary=true.",
            )
